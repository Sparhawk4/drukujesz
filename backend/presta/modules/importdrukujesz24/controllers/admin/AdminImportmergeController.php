...
class AdminImportmergeController extends ModuleAdminController {
	...
	*/
	...
	private function mergeNewProducts($newProducts){
		...
		//echo '<pre>'.print_r($parserProductGroups, true).'</pre>';  die;
		...
		if (Tools::isSubmit('deleteOfferSource')){
			...
			switch($className){
				...
				case 'ExcelParser':
					$this->deleteOfferSource($className);
					break; 					
				case 'ExcelParserPromocje':
					$this->deleteOfferSource($className);
					break; 								
			}		
		}
		if (Tools::isSubmit('submitFileUpload') Or  Tools::isSubmit('submitDownloadFile')){
			...
			if(!$this->errors and Tools::isSubmit('submitFileUpload')){
			...
			if(!$this->errors){
				if(Tools::isSubmit('submitFileUpload')){				
					switch ($className){
						case 'PolcanCSV': 
							$regexPattern  = '/.*(\.csv)|(\.xml)$/i';
							$regexPatternErr = 'The extension of your file should be .csv lub .xml';
						break; 
						case 'ExcelParser': 					
							$regexPattern  = '/.*\.xlsx$/i';
					...
					// nowa wersja adresu do pobierania 
					
					$filePath = 'CennikPolcan_v1_'.date('Y-m-d H:i:s').'.xml';
					//echo basename($filePath); die;
					// http://esklep.polcan.pl/download/CennikPolcan10677160410752562A9EC322-FDA9-47EE-B175-38142BE8E094.xml
					
					$xmlcontent = file_get_contents('https://gw.polcan.pl/edi/rest/TServerMethodsExport/Cennik2XML/DDE72AE0-40C7-48B5-82BF-081C62789236');					
					if($xmlcontent === false){
						$this->errors[] = $this->l('An error occurred while downloading XML file.');							
					}else{
						if( file_put_contents($path.basename($filePath) , $xmlcontent) === false){
							$this->errors[] = $this->l('An error occurred while writing XML file.');															
						}else{								
							$_FILES['file']['name'] = basename($filePath);
							//echo $_FILES['file']['name'];
						}								
					}												
										
					// stara wersja za długo sie generował 
					/*
					$html = file_get_contents('http://esklep.polcan.pl/zapisz_cennik_xml.php?authtoken=DDE72AE0-40C7-48B5-82BF-081C62789236');
					// $html  = file_get_contents('filehhh');
		
					if($html == false){
						$this->errors[] = $this->l('An error occurred while downloading file.');													
					}else{
						// plik wygenerowany teraz pobieranie 
						include_once(_PS_MODULE_DIR_.'importdrukujesz24/classes/HtmlCleaner.php');	
						$html = HtmlCleaner::cleanHtml($html);
						$html = html_entity_decode($html, ENT_COMPAT , 'UTF-8' );	
						
						//file_put_contents('filehhh___' ,  $html);
						//die;
						...
						// div id="
						$nodeList = $xpath->query( '//div[@id="download_info"]'  , $body);						
						//echo $nodeList->length; 						
						$node  = $nodeList->item(0);
						
						$nodeList = $xpath->query( '//a[@class="poz"]'  , $node);						
						// echo $nodeList->length; 	
						$node  = $nodeList->item(0);					
						$href  = $node->getAttribute('href'); 
						
						$filePath  = 'http://esklep.polcan.pl/'.$href;
						//http://esklep.polcan.pl/download/CennikPolcan1067715103107525975A33A18-BED3-4E58-9953-D77D2AAD2BE9.xml
				...
				switch ($className){
					case 'PolcanCSV': 
						...
						if(strtolower($ext) == 'xml'){
							...
							$oferta = new SimpleXMLElement($content /*,LIBXML_NOCDATA*/);
							
							//LT300CL;Brother;Akcesoria;Brother LT-300CL szuflada na papier - 500 arkuszy ;624.06;0;13836
							//LC1100BKBP2;Brother;Materia³y eksploatacyjne do drukarek atramentowych;Tusz Brother do DC6690/MFC5895/6490 | 2 x 450 str. | black;103.09;20+;13629
							//var_dump( $oferta);
							//$oferta = $oferta->oferta;
							//var_dump( $oferta->oferta);  //die;
							//foreach( $oferta->oferta->produkt  as $o ){									
							foreach( $oferta->Produkt  as $o ){		
								//var_dump( $o);
								$lines[] = [(string)$o->kod
										,(string)$o->producent
										,(string)$o->kategoria
						...
						//var_dump( $lines ,$this->errors ,   $fname , $ext) ; die;						
					...
					case 'ExcelParserPromocje':							
						...
						$highestRow         = $worksheet->getHighestRow(); // e.g. 10
						// poszukuje wiersza z opisem zazwyczaj jest to 2 												
						for ($row = 0; $row <= $highestRow; ++ $row) {
							$val = $worksheet->getCellByColumnAndRow(1, $row)->getValue();
							$valP = $worksheet->getCellByColumnAndRow(2, $row)->getValue();
							$val  = strtoupper($val); 
							$valP  = strtoupper($val); 
							if( $val == 'STOCK' AND $valP = 'PRICE' ){
								//echo 'find at ' .   $row;
								break; 								
							}														
						}
						$row++;
						if($row >= $highestRow){
							$this->errors[] = $this->l('Nie znalezino nagłówka w pliku.  Powiny być komórki o wartości STOCK , PRICE');
							die;
						}
						for($row ; $row <= $highestRow; ++ $row) {
														
							$lines[] = [	0 => $worksheet->getCellByColumnAndRow(0, $row)->getValue(), 
							
										3 => $worksheet->getCellByColumnAndRow(0, $row)->getValue(), 
										4 => $worksheet->getCellByColumnAndRow(2, $row)->getValue(), 
										5 => $worksheet->getCellByColumnAndRow(1, $row)->getValue(), 
										6 => $worksheet->getCellByColumnAndRow(0, $row)->getValue(),										
										];	
						}
...
//die;				
				...
				$res =  $db->execute($sql);				
				if(!$res){
					var_dump($res , $db->getNumberError() , $db->getMsgError()   ); 
					die;					
				}		
...
/*
		...
		if (Tools::isSubmit('loadNewProduct')){
			...
			$this->mergeNewProducts($newProducts);	
			$newProducts = $this->getNewProducts();
//echo '<pre>'.print_r($newProducts, true).'</pre>';  die;
			...
			$parser = new $parserClass($this->token);
			$parser->csvArray= $jsonData;	//	echo '<pre>'.print_r($parser->csvArray ,  true).'</pre>'; die;
			$parser->parse();			
...
//echo '<pre>eeee'.print_r($this, true).'</pre>';  die;
