...
/*include_once('PolcanCSV.php');
include_once(_PS_MODULE_DIR_.'/Db.php');
class ExcelParser extends PolcanCSV{
	*/ 
	...
	public  static function getURL($id_xml){
		return('http://braklinku_do_wyznaczenia_adresu='.$id_xml);
	}
	...
	[cena] => 45.60
	[quantity] => 40
	[json] => ["BOXBDR2000N","Croton",null,"Box B\u0119ben zamiennik Brother DR2000",45.6,40,"BOXBDR2000N"]
	[id_product] => 
	[sourceURL] => http://braklinku_do_wyznaczenia_adresu=BOXBDR2000N
	...
	*/
	public  static function findCompatybileProducts($csvProducts){
		...
		$sql =  'SELECT  p.`id_product`, p.`kod_produktu`  ,  LOWER( p.`kod_produktu` )  AS lowkod  FROM `' ._DB_PREFIX_.'product` p WHERE p.`kod_produktu` IS NOT NULL AND  p.`kod_produktu` in ('.$kodyStr . ')';
		...
		if($res){
			foreach($res as $row){
				$values .= '(\'ExcelParser\',\''. pSql($kody[$row['lowkod']]) .'\','. $row['id_product']. '),' ;			
			}
			$values = rtrim( $values , ',');	
	...
	//	echo '<pre>'.print_r($csvProducts, true).'</pre>'; 
	//	die;
	...
	//	die;		
		...
		foreach($this->csvArray as $key => &$row){
			...
			/*if($row[5] === 'stale')
				...
				$row[5] = 20;
				*/ 
			$insertStr .= '(\''. pSQL($row[0]).'\',\''.pSQL($row[1]).'\',\''.pSQL($row[3]).'\','.$row[4].','.$row[5].',\''.pSQL($row[6]).'\'),';
