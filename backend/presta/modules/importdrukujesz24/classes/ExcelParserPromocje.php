...
class ExcelParserPromocje extends AbstractParser{
...
class ExcelParser extends PolcanCSV{
	*/ 
	...
	public  static function getURL($id_xml){
		return('ExcelParserPromocje: '.$id_xml);
	}
	...
	*/
	public  static function findCompatybileProducts($csvProducts){
		...
		$sql =  'SELECT  p.`id_product`, p.`kod_produktu`  ,  LOWER( p.`kod_produktu` )  AS lowkod  
		...
		if($res){
			foreach($res as $row){
				$values .= '(\''.$classname.'\',\''. pSql($kody[$row['lowkod']]) .'\','. $row['id_product']. '),' ;			
			}
			$values = rtrim( $values , ',');	
